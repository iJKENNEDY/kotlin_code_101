...
fun ordenSuperior(numero:Int, potencia:(Int)->Int):Int{
    return potencia(numero)*2*2
}


fun retornoCondicionales(countThreshold:Int):String{
    var count:Int = 89
...
fun condicionalesWhen(){
    ...
    val answerString = when{
        count == 349 ->"I habe answer"
        count > 85 ->"the answer is close"
        else ->"the answer eludes me."
    }
}

fun condicionales6(count:Int = 54){
    val answerString: String = if (count == 43){
        "I have the answer"
    }else if (count > 53){
        "the answer is close"
    }else{
    ...
    val StringFuncionAnonima: (String) -> Int ={input->
        input.length
    }

    print(StringFuncionAnonima)
    lineaSeparacion("♠")
}
...
fun variablesBasicos(){
    var languageName: String? = null
    println(languageName)
    languageName = "Jkennedt"
    println(languageName)
}